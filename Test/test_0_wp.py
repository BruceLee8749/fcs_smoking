...
class TestCase:
    ...
    def test_convert0_021(self):
        ...
        sleep(1)
        """截图验证"""
        driver.screenshot_save(22, sheet_name)
    ...
    def test_convert0_026(self):
        ...
        data = eval(get_cell(fcs_result_path, 27, 4, sheet_name))
    ...
    def test_convert0_051(self):
        """参数wmRotate为90"""
        if get_cell(fcs_result_path, 52, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 52, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
    ...
    def test_convert0_052(self):
        """参数wmPosition为1，1"""
        if get_cell(fcs_result_path, 53, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 53, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
    ...
    def test_convert0_053(self):
        ...
        if get_cell(fcs_result_path, 54, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 54, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
        driver.screenshot_save(54, sheet_name)

    def test_convert0_054(self):
        """水印内容组合"""
        if get_cell(fcs_result_path, 55, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 55, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
        driver.screenshot_save(55, sheet_name)

    def test_convert0_055(self):
        """图片水印组合"""
        if get_cell(fcs_result_path, 56, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 56, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
        driver.screenshot_save(56, sheet_name)

    def test_convert0_056(self):
        """无水印内容组合"""
        if get_cell(fcs_result_path, 57, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 57, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
        driver.screenshot_save(57, sheet_name)

    def test_convert0_057(self):
        """无图片水印组合"""
        if get_cell(fcs_result_path, 58, 7, sheet_name) != '通过':
            pytest.skip("url转换失败,不执行该case")
        url = get_cell(fcs_result_path, 58, 8, sheet_name)
        driver.open_bro(url)
        sleep(10)
        """截图验证"""
        driver.screenshot_save(58, sheet_name)
